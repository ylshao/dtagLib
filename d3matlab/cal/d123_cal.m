...
DEV.ID='3b031d0a';
DEV.NAME='D123';
DEV.BUILT=[2012 10 01];
DEV.BUILDER='TH';
DEV.HAS={'stereo audio'};
BBFILE = ['badblocks_' DEV.ID(1:4) '_' DEV.ID(5:8) '.txt'] ;
try,
   DEV.BADBLOCKS = readbadblocks(['c:/d3/host/d3usb/' BBFILE]) ;
catch,
...
PRESS=struct;
PRESS.POLY=[0 34.25 -1.23];
PRESS.METHOD='rough';
PRESS.LASTCAL=[2012 10 01];
PRESS.TREF = 20 ;
...
ACC.POLY=[5 -2.479; 4.968 -2.521; 4.972 -2.477] ;
ACC.UNIT='g';
ACC.TREF = 20 ;
ACC.TC.POLY=[0; 0; 0];
ACC.PC.POLY=[0; 0; 0];
...
ACC.METHOD='flips';
ACC.LASTCAL=[2012 10 13];

MAG=struct;
MAG.TYPE='magnetoresistive bridge';
MAG.POLY=[691.850 -219.540; 701.020 -213.347; 719.810 -234.820] ;
MAG.UNIT='Tesla';
MAG.TREF = 20 ;
MAG.TC.POLY=[0;0;0];
MAG.TC.SRC='BRIDGE';
