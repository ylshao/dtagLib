...
pbv = apply_cal([X.x{kb}],CAL.PRESS.BRIDGE) ; % first column positive negative voltage difference with poly calib, second column use the temperature calibration to get temperature data
if df>1, pbv = decdc(pbv,df) ; end
% interpolation data to make up for different sampling rate
if X.fs(kb(1))/df<fs,
...
if strcmp(test,'full') || strcmp(test,'tempr'),
   p = apply_cal(p,CAL.PRESS,[],t) ; % get p in m H20 unit
   while ~done,
      CAL = time_select(p,fs,CAL) ; % first plot for choosing satisfactory subset pressure sensor data, add CALTIMESPAN field to CAL.PRESS
      ...
      % default PRESSRANGE = 20, NOTEMP = 0
      [CAL.PRESS,s] = calptc(p(k),t(k),CAL.PRESS,PRESSRANGE,NOTEMP) ; % gui for temperature compensation (second plot)
      done = s~='r' ;
   end
end
...
% default PRESSRANGE = 20, NOTEMP = 0
% p and t data are already truncated 
PRESSDIFF = PRESSRANGE/1000 ;

% select starting set
k = find(p<(min(p)+PRESSRANGE) & abs(diff([0;p]))<PRESSDIFF) ; % truncate large p values and large change in p data 
