...
% and return
% the existing DTAG_123 calibration has TC.POLY, PC.POLY = [0; 0; 0], so
% with this settings, the acceleration data is directly calculated from
% ACC.POLY
...
% truncate data based on selected pressure subset
...
% "s" is the accelerometer raw data from "X"
% "C" is the CAL file with truncate data timespan
for k=1:length(TEST),
   if ~isempty(TXT{k}),
